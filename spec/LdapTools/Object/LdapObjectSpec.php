...
class LdapObjectSpec extends ObjectBehavior
{
    ...
    public function it_should_be_constructable_with_no_attributes()
    {
    ...
    public function it_should_set_the_dn_for_the_batch_collection_on_construction()
    {
        $this->getBatchCollection()->getDn()->shouldBeEqualTo($this->attributes['dn']);
    }

    public function it_should_return_an_array_with_the_exact_attributes_when_calling_to_array()
    {
        $this->toArray()->shouldBeEqualTo($this->attributes);
    }

    public function it_should_allow_me_to_call_a_magical_getter_for_an_attribute()
    {
        $this->getFirstName()->shouldBeEqualTo('Chad');
        $this->getFirstName('default')->shouldNotBeEqualTo('default');
    }

    public function it_should_allow_me_to_call_a_magical_getter_case_insensitive()
    {
        $this->getfIrStnAmE()->shouldBeEqualTo('Chad');
    }

    public function it_should_return_null_when_getting_an_attribute_that_doesnt_exist()
    {
        $this->getFoo()->shouldBeNull();
    }

    public function it_should_return_the_default_value_if_specified_for_an_attribute_that_doesnt_exist()
    {
        $this->get('foo', 'none')->shouldBeEqualTo('none');
        $this->getFoo('none')->shouldBeEqualTo('none');
    }

    public function it_should_return_self_from_a_magic_setter()
    {
        $this->setFirstName('Foo')->shouldReturnAnInstanceOf('\LdapTools\Object\LdapObject');
    }

    public function it_should_set_the_attribute_when_calling_the_magical_setter()
    {
        $this->setFirstName('Foo');
        $this->getFirstName()->shouldBeEqualTo('Foo');
    }

    public function it_should_properly_check_whether_an_attribute_exists()
    {
        $this->has('foo')->shouldBeEqualTo(false);
        $this->has('lastName')->shouldBeEqualTo(true);
    }

    public function it_should_properly_check_whether_an_attribute_with_a_specific_value_exists()
    {
        $this->has('firstName', 'Foo')->shouldBeEqualTo(false);
        $this->has('lastName', 'Sikorra')->shouldBeEqualTo(true);
    }

    public function it_should_be_case_insensitive_when_checking_whether_an_attribute_exists()
    {
        $this->has('LaStNaMe')->shouldBeEqualTo(true);
    }

    public function it_should_get_an_attribute_using_the_magic_property_getter()
    {
        $this->__get('firstName')->shouldBeEqualTo('Chad');
    }

    public function it_should_set_an_attribute_using_the_magic_property_setter()
    {
        $this->__set('firstName', 'foo');
        $this->__get('firstName')->shouldBeEqualTo('foo');
    }

    public function it_should_add_an_additional_attribute_when_calling_the_magical_add()
    {
    ...
    public function it_should_return_self_when_calling_the_magical_add()
    {
        $this->addEmailAddress('Foo.Bar@yay.com')->shouldReturnAnInstanceOf('\LdapTools\Object\LdapObject');
    }

    public function it_should_remove_a_value_when_calling_the_magical_remove()
    {
    ...
    public function it_should_return_self_when_calling_the_magical_remove()
    {
        $this->removeFirstName('Chad')->shouldReturnAnInstanceOf('\LdapTools\Object\LdapObject');
    }

    public function it_should_remove_an_attribute_when_calling_the_magical_reset()
    {
        $this->resetFirstName();
        $this->has('firstName')->shouldBeEqualTo(false);
    }

    public function it_should_return_self_when_calling_the_magical_reset()
    {
        $this->resetFirstName()->shouldReturnAnInstanceOf('\LdapTools\Object\LdapObject');
    }

    public function it_should_implement_a_magic_isset_to_check_for_an_attribute()
    {
        $this->__isset('firstName')->shouldBeEqualTo(true);
        $this->__isset('foo')->shouldBeEqualTo(false);
    }

    public function it_should_allow_getting_an_attriute_with_get()
    {
        $this->get('lastName')->shouldBeEqualTo('Sikorra');
    }

    public function it_should_be_case_insensitive_when_getting_an_attribute()
    {
        $this->get('FirstNamE')->shouldBeEqualTo('Chad');
    }

    public function it_should_allow_setting_an_attribute_using_set()
    {
        $this->set('firstName', 'Foo');
        $this->get('firstName')->shouldBeEqualTo('Foo');
    }

    public function it_should_return_self_when_calling_set()
    {
        $this->set('firstName', 'Foo')->shouldReturnAnInstanceOf('\LdapTools\Object\LdapObject');
    }

    public function it_should_be_case_insensitive_when_setting_an_attribute()
    {
        $this->set('FirstNamE', 'Foo');
        $this->get('firstName')->shouldBeEqualTo('Foo');
    }

    public function it_should_remove_an_attribute_completely_when_calling_reset()
    {
    ...
    public function it_should_be_case_insensitive_when_calling_reset()
    {
        $this->reset('FirsTName');
        $this->has('firstName')->shouldBeEqualTo(false);
    }

    public function it_should_return_self_when_calling_reset()
    {
        $this->reset('firstName')->shouldReturnAnInstanceOf('\LdapTools\Object\LdapObject');
    }

    public function it_should_add_an_attribute_value_when_calling_add()
    {
        $this->add('lastName', 'Bar');
        $this->get('lastName')->shouldContain('Bar');
    }

    public function it_should_be_case_insensitive_when_calling_add()
    {
        $this->add('LaStNaMe', 'Bar');
        $this->get('lastName')->shouldContain('Bar');
    }

    public function it_should_return_self_when_calling_add()
    {
        $this->add('firstName', 'Foo')->shouldReturnAnInstanceOf('\LdapTools\Object\LdapObject');
    }

    public function it_should_remove_an_attribute_value_when_calling_remove()
    {
        $this->remove('lastName', 'Sikorra');
        $this->get('lastName')->shouldBeEqualTo('');
    }

    public function it_should_be_case_insensitive_when_calling_remove()
    {
        $this->remove('LaStNaMe', 'Sikorra');
        $this->get('lastName')->shouldBeEqualTo('');
    }

    public function it_should_return_self_when_calling_remove()
    {
        $this->remove('firstName', 'Chad')->shouldReturnAnInstanceOf('\LdapTools\Object\LdapObject');
    }

    public function it_should_check_if_the_object_is_a_specific_type()
    {
        $this->isType('user')->shouldBeEqualTo(true);
        $this->isType('computer')->shouldBeEqualTo(false);
    }

    public function it_should_add_to_the_batch_collection_for_each_action()
    {
    ...
    public function it_should_return_the_ldap_type_when_calling_get_type()
    {
        $this->getType()->shouldBeEqualTo('user');
    }

    public function it_should_be_able_to_clear_the_batch_modifcations_array()
    {
        $this->addFirstName('Foo');
        $this->removeLastName('Sikorra');
        $this->reset('emailAddress');
    ...
    public function it_should_check_for_an_attribute_when_calling_the_magical_has()
    {
        $this->hasFirstName()->shouldBeEqualTo(true);
    }

    public function it_should_check_for_an_attribute_with_a_value_if_specified_in_the_magical_has()
    {
        $this->hasFirstName('Chad')->shouldBeEqualTo(true);
        $this->hasFirstName('foo')->shouldBeEqualTo(false);
    }

    public function it_should_properly_check_for_an_attribute_with_a_value_even_when_the_attribute_doesnt_exist()
    {
        $this->hasFoo()->shouldBeEqualTo(false);
        $this->hasFoo('bar')->shouldBeEqualTo(false);
    }

    public function it_should_refresh_attributes_without_triggering_a_batch_modification()
    {
        $this->refresh(['firstName' => 'Foo'])->getBatchCollection()->toArray()->shouldHaveCount(0);
        $this->getFirstName()->shouldBeEqualTo('Foo');
    }

    public function it_should_be_case_insensitive_when_refreshing()
    {
    ...
    public function it_should_add_attributes_that_dont_exist_when_refreshing()
    {
        $this->refresh(['foo' => 'bar']);
        $this->getFoo()->shouldBeEqualTo('bar');
    }

    public function it_should_be_able_to_add_multiple_values_at_once()
    {
    ...
    public function it_should_be_able_to_remove_multiple_values_at_once()
    {
    ...
    public function it_should_remove_an_attribute_completely_when_calling_set_with_an_empty_string_empty_array_or_null()
    {
        $attributes = $this->attributes;
        $attributes['foo1'] = 'bar';
        $attributes['foo2'] = 'bar';
