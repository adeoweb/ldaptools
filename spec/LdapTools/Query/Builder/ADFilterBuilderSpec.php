...
class ADFilterBuilderSpec extends ObjectBehavior
{
    ...
    public function it_should_be_an_instance_of_FilterBuilder()
    {
        $this->shouldReturnAnInstanceOf('\LdapTools\Query\Builder\FilterBuilder');
    }

    public function it_should_return_MatchingRule_when_calling_isRecursivelyMemberOf()
    {
        $this->isRecursivelyMemberOf('cn=foo,dc=bar,dc=foo')->shouldReturnAnInstanceOf('LdapTools\Query\Operator\MatchingRule');
    }

    public function it_should_return_MatchingRule_when_calling_hasMemberRecursively()
    {
        $this->hasMemberRecursively('cn=foo,dc=bar,dc=foo')->shouldReturnAnInstanceOf('LdapTools\Query\Operator\MatchingRule');
    }

    public function it_should_use_the_members_attribute_by_default_for_hasMemberRecursively()
    {
        $this->hasMemberRecursively('foo')->getAttribute()->shouldBeEqualTo('members');
    }

    public function it_should_use_the_specified_attribute_when_requested_for_hasMemberRecursively()
    {
        $this->hasMemberRecursively('foo', 'users')->getAttribute()->shouldBeEqualTo('users');
    }

    public function it_should_return_Wildcard_when_calling_mailEnabled()
    {
        $this->mailEnabled()->shouldReturnAnInstanceOf('LdapTools\Query\Operator\Wildcard');
    }

    public function it_should_use_the_groups_attribute_by_default_for_isRecursivelyMemberOf()
    {
        $this->isRecursivelyMemberOf('foo')->getAttribute()->shouldBeEqualTo('groups');
    }

    public function it_should_use_the_memberOf_attribute_if_specified_for_isRecursivelyMemberOf()
    {
