...
class RootDseSpec extends ObjectBehavior
{
    ...
    public function it_should_get_a_LdapObject_for_a_connection()
    {
        $this->get()->shouldReturnAnInstanceOf('\LdapTools\Object\LdapObject');
    }

    public function it_should_have_supported_sasl_mechanisms_as_an_array()
    {
        $this->get()->getSupportedSaslMechanisms()->shouldBeArray();
        $this->get()->hasSupportedSaslMechanisms('GSSAPI')->shouldBeEqualTo(true);
    }

    public function it_should_be_able_to_get_the_default_naming_context()
    {
        $this->get()->getDefaultNamingContext()->shouldBeEqualTo("dc=example,dc=local");
    }

    public function it_should_call_the_schema_load_event_when_getting_the_rootdse_schema($dispatcher)
    {
