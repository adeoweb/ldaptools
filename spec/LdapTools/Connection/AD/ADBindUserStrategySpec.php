...
class ADBindUserStrategySpec extends ObjectBehavior
{
    ...
    public function it_should_return_a_username_in_UPN_format_by_default()
    {
        $this->getUsername('foo')->shouldBeEqualTo('foo@example.local');
    }

    public function it_should_ignore_a_GUID_and_enclose_it_with_curly_braces()
    {
        $guid = '8227ab9b-b307-45eb-a50c-6f6cb3946318';
        $this->getUsername($guid)->shouldBeEqualTo('{'.$guid.'}');
    }

    public function it_should_not_modify_a_SID_username()
    {
        $sid = 'S-1-5-21-1004336348-1177238915-682003330-512';
        $this->getUsername($sid)->shouldBeEqualTo($sid);
    }

    public function it_should_not_modify_a_distinguished_name_username()
    {
        $dn = 'CN=Foo,DC=example,DC=com';
        $this->getUsername($dn)->shouldBeEqualTo($dn);
    }

    public function it_should_not_modify_a_username_in_UPN_form()
    {
        $user = 'foo@bar.com';
