...
class ConvertWindowsTimeSpec extends ObjectBehavior
{
    ...
    public function it_should_return_a_DateTime_object_with_the_correct_time_when_calling_fromLdap()
    {
        $this->fromLdap($this->timestamp)->shouldBeLike(new \DateTime($this->genTime, new \DateTimeZone('UTC')));
    }

    public function it_should_return_a_windows_timestamp_when_calling_toLdap()
    {
        $this->toLdap(new \DateTime($this->genTime, new \DateTimeZone('UTC')))->shouldBeEqualTo($this->timestamp);
    }

    public function it_should_error_when_the_value_going_to_ldap_is_not_a_datetime_object()
    {
